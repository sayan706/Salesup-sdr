...
interface CrmMobileViewProps {
  data: CrmData[]
}

export function CrmMobileView({ data = [] }: CrmMobileViewProps) {
  ...
  // Display the first CRM entry for simplicity
  const crm = data[0] || {}
    ...
    <Card className="border-none shadow-md">
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          <Badge className={`${getStatusColor(crm.status)} text-sm px-3 py-1`}>{crm.status || "Unknown"}</Badge>
        ...
        <div className="mb-4 flex items-center">
          ...
          <div>
            <h2 className="text-lg font-bold">{crm.poc_name || crm.name || "Unknown"}</h2>
        ...
        <div className="text-center p-4 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-500">
            Swipe to see more CRM entries or use desktop view for detailed information.
          </p>
          <p className="text-xs text-gray-400 mt-1">Showing 1 of {data.length} entries</p>
        </div>
      </CardContent>
    </Card>
  )
